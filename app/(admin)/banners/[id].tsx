...
export default function AdminEditBannerScreen() {
    ...
    const loadBanner = async () => {
        try {
            ...
            const banner = data as HeroSlider;
            ...
            Alert.alert('Error', error.message || 'Failed to load banner.');
            router.back();
        } finally {
            setIsLoading(false);
        }
                ...
                { text: 'Cancel', style: 'cancel' },
                {
                    ...
                    onPress: async () => {
                        ...
                        try {
                            ...
                            Alert.alert('Deleted', 'Banner has been deleted.', [
                                { text: 'OK', onPress: () => router.back() }
                            ]);
                        } catch (error: any) {
    ...
    return (
        <SafeAreaView className="flex-1 bg-slate-50" edges={['top']}>
            <View className="px-6 py-4 flex-row items-center justify-between border-b border-slate-200 bg-white z-10" style={{ elevation: 2, shadowColor: '#000', shadowOffset: { width: 0, height: 1 }, shadowOpacity: 0.05, shadowRadius: 2 }}>
                ...
                <Text className="text-xl font-inter-black text-slate-800">Edit Banner</Text>
            ...
            <KeyboardAvoidingView behavior={Platform.OS === 'ios' ? 'padding' : 'height'} className="flex-1">
                <ScrollView className="flex-1" contentContainerStyle={{ padding: 24, paddingBottom: 100 }}>
                    ...
                    <View className="mb-8">
                        <Text className="text-sm font-inter-bold text-slate-700 mb-4 uppercase tracking-wider">Banner Image</Text>
                        <TouchableOpacity
                            onPress={pickImage}
                            className="w-full h-48 rounded-3xl items-center justify-center overflow-hidden border-2 border-dashed border-slate-200 bg-slate-100"
                        >
                            {imageUri ? (
                                ...
                                <View className="items-center">
                                    ...
                                    <Text className="font-inter-medium text-slate-400 mt-2">No Image Selected</Text>
                                </View>
                            )}
                        </TouchableOpacity>
                    </View>
